<?php
    ...
    session_start();        // Sessions are used to store and retrieve data across multiple pages or requests.
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = "SELECT * from railwaydb WHERE from_station = '$from' AND to_station = '$to' AND date='$da'";
        $data = mysqli_query($conn, $query); //Query is being executed and result is stored in data variable

        if (mysqli_num_rows($data) > 0) {
            // if rows are returned from query then the query is stored in the $_SESSION['query1'] variable,
    ...
    // }
        ...
        $query = "SELECT * FROM railwaydb WHERE trainname = '$trainname'";
...
<body>
    ...
    //synth variable is assigned the speechSynthesis object which is  built-in Web API provided by modern web browsers
    let synth = window.speechSynthesis;    
    //utterance object, which represents a speech request,
    let utterance;
    ...
    function speakFrom() {
        let text = `Please enter the source station`;
        speakText(text);
    }

    function speakTo() {
        let text = `Please enter the destination station`;
        speakText(text);
    }
    ...
    // JavaScript function that is used to change the language of a web page by redirecting the user to a different URL based on the selected language.
    function changeLanguage(language) {
        window.location.href = 'form_' + language + '.php';
    }


    //It determines the language spoken by the user. 
    //Based on the recognized language, it calls the changeLanguage function to change the language of the web page.
